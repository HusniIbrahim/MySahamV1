...
public class Main2Activity extends AppCompatActivity {
    ...
    Button button;
    EditText SellPrice,BuyPrice,TotalShare;
    Spinner spinner;
    @Override
    ...
    private boolean validateFields() {
        ...
        if (SellPrice.getText().length() < yourDesiredLength || BuyPrice.getText().length() < yourDesiredLength || TotalShare.getText().length() < yourDesiredLength) {
            if (SellPrice.getText().length() < yourDesiredLength) {
                SellPrice.setError("Your Input is Invalid");
            }
